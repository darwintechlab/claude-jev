...
Mirrors `opencode-openjev`'s 5 tools, for Claude Code via MCP over **live** `https://api.typesafe.ai/v1/systemone`. No mock — `TYPESAFE_API_KEY` required. Tackles Jev downsides with guardrails.
...
| **Hosted dependency** | Live-only: clear error `TYPESAFE_API_KEY is required…` (no silent mock). Retry 429/529/5xx with backoff, 15s timeout. |
...
4. Do not send secrets as `state`. Env: `TYPESAFE_API_KEY` (required), `JEV_MODEL` (default `jev-latest`).
