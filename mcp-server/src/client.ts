...
/** Returns the state to send: unchanged if within the cap, else head+tail truncated. */
function validateState(state: string | object | unknown[]): string | object | unknown[] {
...
export async function decide(state: string | object | unknown[], questions: Questions, opts: ClientOptions = {}): Promise<JevResponse> {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      // Retry only network failures (no response received). A response with a
      // non-retryable status or an unparseable/invalid body will not improve on retry.
